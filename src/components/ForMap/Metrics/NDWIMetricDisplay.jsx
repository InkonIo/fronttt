// components/ForMap/Metrics/NDWIMetricDisplay.jsx
import React, { useState } from 'react';

const NDWIMetricDisplay = ({ activeAnalysisType, layerSelectionBlockWidth, isLayerSelectionBlockExpanded }) => {
  ...
  if (activeAnalysisType !== 'NDWI' || isLayerSelectionBlockExpanded) {
    return null;
  }

  // Цвета и диапазоны значений для NDWI, как в SentinelHubService.java
  const metricItems = [
    { color: '#E6E6B3', range: '-0.3' }, // Сухо/Почва
    { color: '#B3E6B3', range: '0.0' }, // Сухая почва
    { color: '#66CCEC', range: '0.2' }, // Влажная почва
    { color: '#1A66CC', range: '0.5' }, // Вода
    { color: '#001A80', range: '1.0' }, // Макс. вода
  ];
  ...
  return (
    <div style={containerStyle} onClick={() => !isMetricExpanded && setIsMetricExpanded(true)}>
      <div style={titleRowStyle}>
        {isMetricExpanded ? (
          <>
            <h4 style={titleStyle}>Метрика NDWI</h4>
            <button
              onClick={(e) => { e.stopPropagation(); setIsMetricExpanded(false); }}
              style={collapseButtonStyle}
              aria-expanded="true"
              aria-label="Свернуть метрику NDWI"
            >
              {/* Стрелка вверх */}
          ...
          <>
            <span style={{ fontSize: '12px', color: '#f0f0f0', userSelect: 'none' }}>NDWI</span>
            <button
              onClick={(e) => { e.stopPropagation(); setIsMetricExpanded(true); }}
              style={collapseButtonStyle}
              aria-expanded="false"
              aria-label="Развернуть метрику NDWI"
            >
      ...
      {isMetricExpanded && (
        <>
          <p style={{ margin: '0 0 8px 0', fontSize: '10px', color: '#bbb', textAlign: 'center' }}>
            Используется для выделения водных объектов.
          </p>
...
export default NDWIMetricDisplay;
