...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const unauthenticatedOnlyPaths = ['/login', '/register', '/demo-login']; 
    ...
    console.log('Is Current Path an Unauthenticated-Only Path:', unauthenticatedOnlyPaths.includes(location.pathname));
    ...
    if (!isAuth) {
      ...
      if (unauthenticatedOnlyPaths.includes(location.pathname)) { 
        console.log('App.jsx - Redirecting to /dashboard: Authenticated and on an unauthenticated-only path.');
